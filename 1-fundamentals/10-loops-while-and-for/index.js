...
for (let i = 0; i < 5; i++) {
    console.log(i % 2 === 0 ? i : 'impair');
}

for (let counter = 0; counter < 20; counter++) {
    if (counter % 3 === 0 || counter % 2 === 0) continue;
    console.log(counter);
}
...
const usersToObj = JSON.parse(jsonUsers);
const mapUsers = usersToObj.map(user => {
    return {
        id: user.id,
        name: user.name,
...
const userNames = [];
for (let i = 0; i < usersToObj.length; i++) {
    const user = usersToObj[i];
